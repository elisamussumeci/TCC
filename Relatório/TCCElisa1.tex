...
O processo de formação de opinião\todo{nos dias de hoje,} é fortemente influenciado pela mídia digital.
...
o papel das mídias digitais no acesso à informação.\todo{incluir aqui um parágrafo sobre os tipos de mídia digital}\todo{falar também que midias diferentes tem periodicidades, alcances , publicos e credibilidades diferentes e que isso afeta a disseminação da informação }




A mídia tem um forte papel influenciador no processo de formação de opinião. Sendo assim, podemos considerar 
importante entender como uma notícia se\todo{se forma e se espalha na internet} comporta na mídia

A partir dos dados do Projeto Media Cloud Brasil\todo{referência}, estudamos ccomo uma notícia se dissemina na mídia. Para isso
...
\todo[inline]{Esplicar o que é um processo de contágio, e explicar por que você acha que o espalhamentoo das notícias é um. Falar do modelo SIR como um modelo que pode representar a dinâmica das notícias.}
...
\todo[inline]{falar sobre o critério de construção da rede, usando tempo e similaridade, para aproximar a rede real, não observável.}
...
(AJEITAR MATRIZ)
\todo{aqui talvez seja melhor usar o ambiente array, ao invés de tabular}
\begin{center}
 \begin{tabular}{ccccc}
...
Temos nessa matriz a representação de todas as palavras presentes em nosso corpus, porém, queremos representar documentos e não palavras.\todo{como é esta representação?, porque ela é adequada para o seu trabalho? explicar isso em uma seçãoo sobre o skipgram no referencial teórico}
...
$Idf_{t}$.\todo{mover a explicação do TF-IDF para o a seção \ref{sec:nlp}, referencie a equação do tf-IDF aqui e deixe na metodologia apenas o que você fez, usando o TF-IDF e outras coisas mais.}
 ...
 \todo{todas as equações devem estar em ambientes equation para serem referenciáveis.}
...
\todo{Você copiou este texto de algum lugar?}
