...
Para isso
estudaremos os caminhos percorridos em uma rede de disseminação \todo{Estimada a partir da} base de dados do Projeto MediaCloud Brasil\footnote[1]{\url{https://github.com/NAMD/mediacloud_backend}}.
...
eficiente do modelo \hyperref[sec:nlp]{skip-gram} e \hyperref[sec:nlp]{bag-of-words} contínuo.\todo{preciso explicar mais sobre o word2vec? Sim, aqui, por exemplo s\'o usamos o skipgram e n\~ao o CBOW. Al\'em disso \'e preciso dizer os par\^ametros usados para treinar o modelo.}

Para gerar nosso modelo Word2Vec \todo{preciso explicar que tem que haver um conjunt trainado com todos os artigos do mediacloud etcs? Sim tem que dizer que o modelo foi treinado com n documentos, considerando bigramas (?)}, fornecemos como entrada nosso corpus linguístico
...
\subsubsection{Construção da Rede}
 ...
 \end{description}
 \todo[inline]{Elisa, aqui t\^em que mostrar as distribui\c{c}\~oes e dizer quais foram os valores}
...
\todo{incluir distribuição de tempo}
\begin{figure}[ht]
 \centering
...
que o $\lambda$ crítico do modelo é dado por: \todo{dizer que $\lambda_c$ corresponde a $R_0=1$}
...
\todo{rodar o codigo para pegar esses valores}
\begin{table}
